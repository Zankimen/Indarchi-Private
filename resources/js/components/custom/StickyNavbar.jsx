...
const StickyNavbar = ({ children, className }) => {
  return (
    <nav className={cn("sticky top-0 z-50 w-full bg-background dark:bg-[#1e1b4b] border-b border-border shadow", className)}>
...
const NavbarItem = ({ children, active = false, onClick, className }) => {
  return (
    <button
      onClick={onClick}
      className={cn(
        "transition-colors hover:text-secondary cursor-pointer font-bold",
        active ? "text-secondary border-b-2 border-secondary pb-1 dark:text-foreground dark:border-foreground" : "text-muted-foreground hover:text-secondary dark:text-foreground",
        className,
      )}
    >
      {children}
