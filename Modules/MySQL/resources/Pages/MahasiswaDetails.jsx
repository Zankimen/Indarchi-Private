...
function MahasiswaDetails() {
  const { mahasiswa } = usePage().props;

  return (
    <>
      <Head title={mahasiswa.nama} />
      <div className="space-y-4">
        <Card className="border-border">
          <div className="grid grid-cols-1 sm:flex sm:justify-between items-center px-6 py-2 gap-4">
            ...
            <div className="grid grid-cols-1 gap-2 sm:flex">
              ...
              <Link href={`/mysql/mahasiswa/edit/${mahasiswa.id}`}>
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    confirm("Are you sure you want to delete this Mahasiswa?")
                  ) {
                    router.delete(`/mysql/mahasiswa/${mahasiswa.id}`);
                  }
                }}
              >
        ...
        <Card className="p-6 px-8 border-border">
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 px-2">
            <Detail label="NIM" value={mahasiswa.nim} />

            <Detail label="Nama" value={mahasiswa.nama} />

            <Detail label="Alamat" value={mahasiswa.alamat} />

            <Detail
              label="Created At"
              value={formatDateNoHour(mahasiswa.created_at)}
            />

            <Detail
              label="Last Updated At"
              value={formatDateNoHour(mahasiswa.updated_at)}
            />
          </div>
        </Card>
      </div>
...
MahasiswaDetails.layout = (page) => <Dashboard children={page} />;

export default MahasiswaDetails;
